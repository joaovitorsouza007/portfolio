...
const IMAGEM_PLACEHOLDER =
  ...
  encodeURIComponent(
    '<svg xmlns="http://www.w3.org/2000/svg" width="600" height="400"><rect width="100%" height="100%" fill="#1a1e29"/><text x="50%" y="50%" fill="#00784b" font-family="sans-serif" font-size="28" text-anchor="middle" dominant-baseline="middle">Sem imagem de capa</text></svg>'
  );
...
const navegacao = document.getElementById("navbarNav");
navegacao.addEventListener("click", (e) => {
  if (!e.target.closest(".nav-link")) return;
  bootstrap.Collapse.getOrCreateInstance(navegacao, { toggle: false }).hide();
});
...
function renderizarProjetos() {
  if (!projetos.length) {
    const proximoPasso = logado
      ? 'Use o botão "Novo projeto" para publicar o primeiro.'
      : "Fale comigo pelos canais de contato abaixo.";
    listaEl.innerHTML =
      `<div class="col-12"><p class="text-center text-muted mb-0">Nenhum projeto publicado ainda. ${proximoPasso}</p></div>`;
    return;
  }
      ...
      const adminBotoes = logado
        ...
        <div class="d-flex gap-2 mt-2">
          ...
          <button class="btn btn-outline-dark btn-sm w-100" data-excluir="${p.id}">
            <i class="bi bi-trash3-fill me-1" aria-hidden="true"></i>Excluir
          </button>
        </div>`
        : "";
      return `
      <div class="col-md-6 col-lg-4">
        <div class="card h-100 shadow-sm">
          ...
          <div class="card-body d-flex flex-column">
            <h3 class="h5 card-title">${escaparHtml(p.titulo)}</h3>
            ...
            <div class="d-flex align-items-center gap-2 mt-auto mb-2">
              ${p.demo ? `<a href="${p.demo}" class="btn btn-primary flex-grow-1" target="_blank" rel="noopener" data-clique="${p.id}"><i class="bi bi-box-arrow-up-right me-1" aria-hidden="true"></i>Demo</a>` : ""}
              ${p.repositorio ? `<a href="${p.repositorio}" class="btn btn-outline-dark" target="_blank" rel="noopener" data-clique="${p.id}"><i class="bi bi-github me-1" aria-hidden="true"></i>Repositório</a>` : ""}
            </div>
            ${adminBotoes}
          </div>
        </div>
...
function renderizarProvaReal() {
  const v = DEPOIMENTO_VERIFICADO;
  const provaEl = document.getElementById("prova-real");
  if (!provaEl) return;
  provaEl.innerHTML = `
    <div class="col-md-6 col-lg-4">
      <div class="card h-100 shadow-sm">
        <div class="card-body d-flex flex-column">
          <div class="d-flex justify-content-between align-items-start gap-2">
            <i class="bi bi-quote card-quote text-muted" aria-hidden="true"></i>
            <a class="badge badge-verified" href="${v.projetoDemo}" target="_blank" rel="noopener" title="Abrir o projeto ${escaparHtml(v.projetoTitulo)}">
              <i class="bi bi-patch-check-fill me-1" aria-hidden="true"></i>Cliente verificado
            </a>
          </div>
          <p class="card-text mt-2">${escaparHtml(v.texto)}</p>
          <div class="d-flex align-items-center gap-3 mt-auto pt-3 border-top">
            <img src="${v.foto || avatarIniciais(v.nome)}" alt="${v.foto ? `Foto de ${escaparHtml(v.nome)}` : `Avatar com a inicial de ${escaparHtml(v.nome)}`}" class="comment-avatar rounded-circle" loading="lazy" decoding="async">
            <div>
              <h3 class="h6 mb-0 fw-bold">${escaparHtml(v.nome)}</h3>
              <small class="text-muted">${escaparHtml(v.cargo)}</small>
            </div>
          </div>
        </div>
      </div>
...
window.addEventListener("hashchange", () => {
  if (window.location.hash === "#admin") abrirLogin();
});
...
function atualizarInterfaceAuth() {
  ...
  btnAdmin.classList.toggle("d-none", !logado);
  document.getElementById("admin-link").classList.toggle("d-none", logado);
  renderizarProjetos();
  ...
  document.getElementById("ranking-projetos").innerHTML = ranking
    .map(
      ...
      <li class="list-group-item d-flex justify-content-between align-items-center ${i === 0 && r.cliques > 0 ? "fw-bold" : ""}">
        <span>${escaparHtml(r.titulo)} ${i === 0 && r.cliques > 0 ? '<span class="badge badge-skill ms-2"><i class="bi bi-star-fill me-1" aria-hidden="true"></i>Mais clicado</span>' : ""}</span>
        <span class="badge badge-skill-outline">${r.cliques} clique${r.cliques === 1 ? "" : "s"}</span>
      </li>`
    )
    .join("");
...
const STORAGE_COMENTARIOS = "comentarios:v2";

const DEPOIMENTO_VERIFICADO = {
  id: "graziela-souza",
  nome: "Graziela Souza",
  ...
  foto: null,
  projetoTitulo: "Portfólio Graziela Souza",
  projetoDemo: "https://graziela-portfolio.vercel.app/"
};
...
function carregarComentarios() {
  ...
  try {
    ...
    const listaMigrada = lista.filter((c) => c && c.id !== "c2");
    if (listaMigrada.length !== lista.length) {
      localStorage.setItem(STORAGE_COMENTARIOS, JSON.stringify(listaMigrada));
    }
    return listaMigrada;
  } catch {
    return [];
...
function renderizarComentarios() {
  if (!comentarios.length) {
    const vazio = logado
      ? `<p class="text-muted mb-0">Nenhum depoimento salvo neste navegador. Depoimentos são guardados por navegador, sem servidor: o que visitantes publicam não chega até esta lista.</p>`
      : "";
    listaComentariosEl.innerHTML = `<div class="col-12"><div class="text-center" role="alert">${vazio}</div></div>`;
    return;
  }
    ...
    .map((c) => {
      const blocoResposta =
        ...
        <div class="comment-reply mt-3">
          <small class="fw-bold text-accent"><i class="bi bi-reply-fill me-1" aria-hidden="true"></i>João Vitor Souza <span class="badge badge-author ms-1">Autor</span></small>
      ...
      const adminBotoes = logado
        ...
        <div class="d-flex gap-2 mt-3">
          <button class="btn btn-outline-primary btn-sm w-100" data-responder="${c.id}">
            <i class="bi bi-reply-fill me-1" aria-hidden="true"></i>${c.resposta && c.resposta.texto ? "Editar resposta" : "Responder"}
          </button>
    ...
    <div class="col-md-6 col-lg-4">
      <div class="card h-100 shadow-sm">
        <div class="card-body d-flex flex-column">
          ...
          <div class="d-flex align-items-center gap-3 mt-auto pt-3 border-top">
            <img src="${c.foto || avatarIniciais(c.nome)}" alt="${c.foto ? `Foto de ${escaparHtml(c.nome)}` : `Avatar com a inicial de ${escaparHtml(c.nome)}`}" class="comment-avatar rounded-circle" loading="lazy" decoding="async">
            <div>
              <h3 class="h6 mb-0 fw-bold">${escaparHtml(c.nome)}</h3>
...
function excluirComentario(id) {
  const c = comentarios.find((com) => com.id === id);
  if (!c) return;
  if (!confirm(`Excluir o depoimento de "${c.nome}"? Essa ação não pode ser desfeita.`)) return;
